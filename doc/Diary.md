...
- さらにAVRの電源未接続も発見。KiCadのATmega128のシンボルライブラリが、なぜかPin52,63Pが定義されているにもかかわらず非表示にされていた。64pinの割に電源とGNDが一対しかないのが気になっていたが、KiCadのシンボルを信じてデーターシートをちゃんと確認していなかった。リプログラムが不安定になるのは、もしかしてこれが原因だったのかも。

## 2023/9/18
- 9/16からの連休を使って基板を完成させた。作業を以下にまとめておく。

### KiCadのPCBの作成手順
- ざっと以下の手順を実行した。
1. 回路図エディタの`検査 > エレクトリカル ルール チェッカー(ERC)`でエラーがないことを確認する。
2. 回路図エディタの`ツール > 回路図から基板を更新`でPCBを生成。
3. PCBエディタの`検査 > デザインルール チェッカー`(DRC)でエラーがないことを確認する。
4. フットプリントを配置する。
   - おおよその位置が決まったら基板外形を決める。以後は、この中に収まるように作業する。
5. 基板設定
   - 基板層数 (2層、4層など)
   - 制約(配線幅、クリアランスなど)
6. ベタの設定
   - 4層なら3層目をVCCに、表面、2層目、裏面をGNDベタの設定を行う。
7. 手配線
...
- ベタ
  - 設定
    - `塗りつぶしゾーンを追加`で、ベタを設定する領域を指定する。複数のレイヤーがまとめて指定できるので、ベタGNDの設定はこれが便利。
  - 除外
