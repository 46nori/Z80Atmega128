...
- BIOSのBOOTからCCPを起動(CCPのエントリ+3にジャンプ)するが、ディレクトリの読み込み後に、コンソール出力に`^@`(制御コード)が延々と表示される。この問題の原因がさっぱりわからない。Z80側のデバッグはOUT命令くらいしか使えないので、AVR側からZ80にブレークポイントを張れるようにした。

#### ブレークポイント機能の追加
- 簡易モニタに以下のコマンドを追加した。
  - `brk <adr>` : <adr>に0xffを書き込む。RST 7なので実行されると0x38にトラップする。ここにはBIOS内のデバッグルーチンにジャンプし、HALT状態になる。
  - `del <adr>` : <adr>に書き込んだ命令を元に戻し、ブレークポイントを削除する。
  - `cont` : AVRから割り込みがかかり、HALTが解除される。直前のブレークポイント位置に復帰する。復帰先の命令は0xffのままなので、実行前に`del`で命令の復帰とブレークポイント削除を行っておくこと。
  - `ib` : ブレークポイントの一覧(アドレスとその位置の命令)を表示する。
- ブレークするとAVRのコンソールに`>>>Break! (ブレークアドレスの下位バイト)`が表示される。