...
故障した基板の修理を行なった。
- **Z80, HM62256(SRAM)は完全死亡で交換。**
- **ATmega128は半死状態で交換。** AVR側モニタは動いていたが、Z80側のシリアルが壊れていた。
  - Fuseの書き換え、ファームの書き込み、EEPROMにCP/M BIOSの書き込みを実施。
- DS1232や74AC573は生きていた。
- **その他の74系標準ロジックは念の為交換。**
- レギュレータで3.3Vを供給している74HC4030やSDカード周りは問題なし。

ATmega128はQFPなので交換が大変。自動はんだ吸取機はないよりマシ程度であまり使い物にならない。
グラインダでチップの足を削ったが、一歩間違うと基板に致命的なダメージを与えるのでやめた方が良い。
さらにハンダこてでピンを温めながら力をかけてはぎとった。うまく行ったがこれは運みたいなもの。やはりヒードガンが欲しい。

## 2026/3/28

- 3/23から対応していた[バグ修正計画1](./bugfix/BugFixPlan1.md)の内容は動作確認用ブランチ `bugfix/BUG_working` にマージ済み。
- 特に問題なさそうなので、developブランチにマージし一旦完了。
