...
|用語  |正式名称| 役割                                                      |
|------|-------|-----------------------------------------------------------|
| IPL  | Initila Program Loader | CP/Mの管理外なので実装方法規定されないが、必要な情報をメモリ上にロードしBIOSを起動する。|
| BIOS | Basic I/O System | 物理ハードウェアを制御するAPIライブラリ。IPLから起動され、CCPを呼び出すことでCP/Mを起動する。以後、BDOSから渡される、パラメータ(論理ディスク、トラック、セクタ)を[DPB](#DPB(Disk Parameter Block))の内容に応じて物理トラック、セクタに変換し、実際にディスクの読み書き、およびコンソールI/Oの処理を行う。|
...
|用語      |                                                                 |
|----------|-----------------------------------------------------------------|
|物理ディスク|特性は[DPB(DISK Parameter Block)](#DPB(Disk Parameter Block))で定義される。                   |
...
|論理トラック|論理ディスクのトラック。<br>[DPB](#DPB(Disk Parameter Block))のOFFが加算されているが、OFF分はBDOSからはアクセスされない。|0-65535|
...
|パラメータ|Bytes|                              ||
|:----:|:---:|---------------------------------|---------------------------|
| XLT  | SPT | トラックあたりの、論理セクタと物理セクタのマッピングテーブル。スキューテーブルともいう。フロッピーディスクのように物理的に回転する媒体を読み書きする場合、物理セクタと論理セクタの対応ずらしておくことで平均アクセス速度を改善するテクニックをスキューと呼ぶ。物理動作の不要な媒体では不要なテクニック。|
| DIRB | 128 | 固定値 |
| CSV  | CKS | [DPB](#DPB(Disk Parameter Block))のCKSを参照 |
| ALV  | DSM / 8 + 1 | [DPB](#DPB(Disk Parameter Block))のDSMから算出する。|
...
| BSH |  1  | Block SHift factor              |ブロックサイズから決まる値。([下表](#BLS/BSH/BLM/EXMの関係))<br>BLS = 128 $\times$ $2^{BSH}$|
| BLM |  1  | BLock Mask                      |ブロックサイズから決まる値。([下表](#BLS/BSH/BLM/EXMの関係))<br>BLS = 128 $\times$ (BLM + 1)|
| EXM |  1  | EXtent Mask                     |ブロックサイズとDSMから決まる値。([下表](#BLS/BSH/BLM/EXMの関係))|
...
参考までに[cpmtools](http://www.moria.de/~michael/cpmtools/)のmkfs.cpmで使用するdiskdefのパラメータとの対応関係も記載した。
