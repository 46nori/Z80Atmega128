...
以下にSRAM(HM62256)の品種と、必要なWAITの関係を示す。
|             | $t_{AA}$ | $t_{OE}$ | WAIT |
| ----------- | ---------| -------- | ---- |
...
したがってSRAMの型番はREADのタイミング制約に合わせて決定すればよい。
...
* 最終的な値は以下となる。上記以外のbitの設定はデフォルトのままである。  
  値の変更は、Microchip StudioのDevice ProgrammingメニューからISP経由で行える。
    |                    |  Value  |(Default)|
    |--------------------|---------|---------|
...
Z80はリセット時に0番地からプログラムを実行するため、AVRはSRAMの$0000にブートストラップコードが配置しておかなければならない。しかしAVRではSRAMは内部メモリの直後に配置されるので、$0000-$10FFの空間には直接アクセスすることができない。この問題はSRAMのアドレス空間を一時的に狭めることで発生するシャドウ効果を利用した間接的なメモリアクセスで回避できる。　　

XMEMインターフェース有効時、PortCはアドレスバスの上位8bitにマップされるが、XMCRBのXMM0/1/2の設定で一部のピンをGPIOとして解放することが可能である。例えばPortCの上位3bitをGPIOとして解放し、強制的に0出力とすることで、SRAMの$0000-$1FFFが、AVRの$2000-$3FFFのメモリ空間にマップできる。
