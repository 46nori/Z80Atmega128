...
システムにリセットがかかりAVRの初期化が完了するまでは、Z80は起動できない。Z80が実行するSRAMの0番地からのコードは、AVRが初期化コード内でセットするからである。このためリセットパルスの幅をZ80>AVRとなるようにし、AVRの初期時間を確保する。

回路を簡略化するため、パワーオンリセットと、マニュアルリセットを同時にサポート可能な[DS1232N](https://akizukidenshi.com/catalog/g/gI-15313/)を利用している。これを2つカスケード接続することで、AVR、Z80のリセットパルスを生成する。
DS1232Nのリセットパルス幅の最小値は250msなので、AVR、Z80のリセットパルス幅はそれぞれ250ms、500msとなる。この250msの時間差内で、AVRはペリフェラルのセットアップとZ80のプログラムの配置を行う。実際には0番地にHALT命令を書き込むことでZ80が起動を開始してもHALT状態にさせておき、プログラムの書き込み後にAVRからZ80にリセット信号を送って再起動させる。

なお、リセット直後/BUSREQがイネーブルであっても、Z80は0番地からの1命令を実行してからバスをリリースする。つまりZ80の起動タイミングをAVRより遅らせるには/BUSREQでは不足で、/RESETをLowに保持したまま起動を止めておく必要がある。

DS1232Nはウォッチドックタイマー機能があるが、OFFすることができない。一定時間に毎に/STにパルスを投入する必要があるため、Z80用の4HMzのクロック信号を入力している。

AVRがISPによる制御モード下にある場合、リセット信号はLowのまま保持される。[ISP接続時のReset Lineのプルアップ抵抗の制限](https://onlinedocs.microchip.com/pr/GUID-BEB648AA-8539-418A-9EFB-118BDC9DC77B-en-US-1/index.html?GUID-28E6AE85-17D2-45FA-A3CC-FE1B4DC3E00F)により、R>4.7Kにする必要がある。10Kでプルアップしている。
...
本回路では、Z80からAVRへのI/Oサイクル、およびAVRからZ80への割り込み要求で、AVRに割り込みを発生させる。AVRの割り込みハンドラ内では/BUSREQの要求をしてはならない。これはZ80のI/Oサイクル中、あるいは割り込みサイクル中は、AVRはSRAMにアクセスできないことを意味する。
Z80は現在実行中のマシンサイクルが終了するタイミングで/BUSREQを受け付ける。つまり、割り込みハンドラ内で自動挿入される/WAITをリセットして現在のマシンサイクルが完了させる前に/BUSREQを要求すると、永久に受け付けられずにデッドロックに陥る。
