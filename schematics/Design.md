...
AVRがDMAを行う場合、はじめにZ80に対し/BUSRQをLowにして使用権を要求する。Z80は/BUSACKをLowにすることでパスの使用権をAVRに明け渡したことを知らせる。このときZ80の入出力信号はハイインピーダンスになる。AVRはバスの使用が完了したら/BUSRQをHighにし、バスの使用権をZ80に戻す。

本回路では、Z80からAVRへのI/Oサイクル、およびAVRからZ80への割り込み要求で、AVRに割り込みが発生する。これらに対するAVRの割り込みハンドラ内では/BUSREQの要求をしてはならない。これはZ80のI/Oサイクル中、あるいは割り込みサイクル中は、AVRはSRAMにアクセスできないことを意味する。
Z80は現在実行中のマシンサイクルが終了するタイミングで/BUSREQを受け付ける。つまり、割り込みハンドラ内で自動挿入される/WAITをリセットして現在のマシンサイクルが完了させる前に/BUSREQを要求すると、永久に受け付けられずデッドロックに陥る。
...
* PB5  
  常に出力にセットする。Z80への/RESETパルス生成用。
* PB6  
  常に入力にセットする。Z80への/NMI割り込みパルス生成用。
* PB7  
